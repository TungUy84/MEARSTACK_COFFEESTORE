...
const DetailProduct = () => {
    ...
    // Lọc 3 sản phẩm khác không trùng sản phẩm hiện tại
    const otherProducts = [];
    let i = startIndex;
    ...
    if (!product) {
      return (
        <div className="text-center mt-20">
          <h2 className="text-2xl font-bold text-red-600">Sản phẩm không tồn tại!</h2>
          <Link to="/" className="text-blue-600 underline mt-4 inline-block">Quay về trang sản phẩm</Link>
        </div>
      );
    }
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 text-[#3d1f00] bg-white">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-700 mb-6 leading-relaxed">
              {product.description || "Thức uống đặc biệt, thơm ngon cho mọi lứa tuổi."}
            </p>
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-20 border-2 border-gray-300 rounded px-2 py-1"
              />
            </div>
        ...
        <div className="mt-12">
          ...
          <div className="flex items-center gap-4">
            <button onClick={handlePrev} className="text-xl px-3 py-2 bg-gray-200 rounded hover:bg-gray-300">&#8592;</button>
            ...
            <div className="flex gap-4 w-full">
              {otherProducts.map((item) => (
                ...
                >
                  <img src={`/images/${item.image}`} alt={item.name} className="h-32 w-full object-cover rounded mb-2" />
                  <p className="text-sm font-medium">{item.name}</p>
                  <p className="text-sm text-gray-500">{parseInt(item.price).toLocaleString()} VNĐ</p>
                </Link>
              ))}
            </div>
  
            <button onClick={handleNext} className="text-xl px-3 py-2 bg-gray-200 rounded hover:bg-gray-300">&#8594;</button>
          </div>
        </div>
      </div>
    );
